...
class V32VehicleSensorsDataTranslator(object):
    ...
    def __init__(self, serial_data, vehicle_event_dispatcher):
        ...
        vehicle_event_dispatcher.dispatch_event(VehicleEventDispatcher.GYRO_RAW_DATA_EVENT, gyro_data)
        
        accel_data = Vector3D(splitted_data[3],splitted_data[4],splitted_data[5])
        vehicle_event_dispatcher.dispatch_event(VehicleEventDispatcher.ACCEL_RAW_DATA_EVENT, accel_data)
        
        mag_data = Vector3D(splitted_data[6],splitted_data[7],splitted_data[8])
        vehicle_event_dispatcher.dispatch_event(VehicleEventDispatcher.MAGNETOMETER_RAW_DATA_EVENT, mag_data)
